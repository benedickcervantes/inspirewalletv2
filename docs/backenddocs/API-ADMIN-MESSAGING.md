...
These endpoints allow admins to send support messages to users, list conversations, and search users for recipient selection. All require `Authorization: Bearer <admin_token>` and ADMIN role.
...
List all admin-to-user conversations with latest message summary.
