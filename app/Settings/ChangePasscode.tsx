...
function MessageModal({ visible, onClose, title, message, confirmText = 'OK', onConfirm, success }: MessageModalProps) {
...
export default function ChangePasscode() {
  ...
  const currentPin =
    step === 'current' ? currentPasscode : step === 'new' ? newPasscode : confirmPasscode;
  ...
  const submitChange = async () => {
    ...
    setLoading(true);
    const result = await updatePasscode(accessToken, currentPasscode, newPasscode);
    setLoading(false);
              ...
              <View style={styles.padRowLast}>
                ...
                <TouchableOpacity
                  style={[
                    styles.padButton,
                    styles.padButtonDel,
                    { width: delBtnSize, height: delBtnSize, borderRadius: delBtnSize / 2 },
                  ]}
